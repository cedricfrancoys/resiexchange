...
<div class="stories" id="xnews-{$link_shakebox_index}" {* if $link_shakebox_currentuser_reports gt 0} style="opacity:0.5;filter:alpha(opacity = 50)"{/if *}>
	...
	<div class="story_data">
		...
		{else}
            {if $pagename eq "new"}
                <div class="votebox votebox-new {if $link_shakebox_comments gt 0}votebox-answers{/if}"> 
                    <div class="vote" title="{$link_shakebox_comments} answers"><div class="votenumber">{$link_shakebox_comments}</div><div class="votebutton"><i class="fa fa-black fa-comments" aria-hidden="true"></i></div></div>
                </div>
                <div class="votebox votebox-new {if $link_shakebox_votes gte 5}votebox-votes{/if}"> 
                    <div class="vote" title="{$link_shakebox_votes} votes"><div class="votenumber">{$link_shakebox_votes}</div><div class="votebutton"><i class="fa fa-check-circle" style="{if $link_shakebox_currentuser_votes eq 1}color: green;{/if}" aria-hidden="true"></i></div></div>
                </div>
                <div class="votebox votebox-new"> 
                    <div class="vote" title="{$link_shakebox_views} views"><div class="votenumber">{$link_shakebox_views}</div><div class="votebutton"><i class="fa  fa-eye" aria-hidden="true"></i></div></div>
                </div>                
            {else}
				...
				{if $use_title_as_link eq true}
				 ...
				 {else}
					...
					{else} 
                      ...
                      <a href="{$title_url}">{$title_short}</a>
					{/if}
				{/if}
				...
				{if $enable_tags}
					{if $tags}
						...
						{section name=thistag loop=$tag_array}
							{if $tag_array[thistag] neq '' && $tag_array[thistag] neq 'Tous'}
...
$(document).ready(function(){
    $(".comment-content").each(function(i, elem) {
        var src = $(elem).text();
        var html = markdown.toHTML(src);
        $(elem).html(html);
    });    
});  
</script>
