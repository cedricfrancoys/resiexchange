...
<form action="" method="post" id="thisform" enctype="multipart/form-data" >
	...
	*}
	...
	{if $enable_tags}
        ...
        <select tabindex="10" id="tags" name="tags" value="{$tags_words}" class="form-control col-md-4" multiple="multiple" value="{$tags_words}" autocomplete="off">
        <option value="{$tags_words}" selected="selected">{$tags_words}</option>
        </select>        
		<br />
	{/if}
	...
	<br />
	<textarea name="bodytext" rows="25" id="bodytext" data-provide="markdown" class="form-control" WRAP="SOFT">{$submit_content}</textarea>
