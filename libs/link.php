...
/////
	...
	function read_basic() {
		...
		$id = $this->id;
		if(!is_numeric($id)){return false;}
		if(($link = $db->get_row("SELECT link_comments, link_views, link_author, link_status, link_randkey, link_category, link_date, link_votes, link_karma, link_published_date,link_group_id FROM " . table_links . " WHERE link_id = $id"))) {
			$this->author=$link->link_author;
			$this->votes=$link->link_votes;
            $this->views=$link->link_views;
			$this->karma=$link->link_karma;
	...
	function fill_smarty($smarty, $type='full'){
		...
		if($type == "summary"){
			if($this->link_summary == ""){
				$smarty->assign('story_content', $Parsedown->text($this->truncate_content()));
			} else {
				$smarty->assign('story_content', $Parsedown->text($this->link_summary));
			}
		}
		if($type == "full"){
			$smarty->assign('story_content', $Parsedown->text($this->content));
		}
        ...
        $smarty->assign('link_shakebox_views', $this->views);
        $smarty->assign('link_shakebox_comments', $this->comments);
